...
const { isAbsolute, resolve: resolvePath, extname } = require('node:path');
...
function mdLinks(path, options = { validate: false, stats: false }) {
  return new Promise((resolve, reject) => {
    ...
    if (!isAbsolute(path)) { //  2.si path no es absoluta, lo convierto en absoluta
      absolutePath = resolvePath(path) // llamo a la función resolvePath(path) es el path que convierto de relativo a absoluto
    ...
    if (stat.isDirectory()) {  //  4.verifico si path es archivo o directorio.
      populateMDFiles(absolutePath);
      const promises = files.map(file => mdLinks(file, options))
      Promise.all(promises).then(result => {
        let links = [];
        result.forEach(fileLinks => links = links.concat(fileLinks))
        resolve(links)
      })
      return;
    } else {
...
function populateMDFiles(path) {
  const stat = fs.statSync(path);
  if (stat.isDirectory()) {
    fs.readdirSync(path).forEach((subpath) => {
      populateMDFiles(path + `/${subpath}`)
    })
  } else {
    if (mdExtension(path)) {
      files.push(path);
